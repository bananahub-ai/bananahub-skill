...
在支持隐式加载 skill 的宿主里，普通“生成图片 / 画一个”的请求也可以触发 BananaHub。此时 BananaHub 会先询问是否需要优化 prompt；如果你拒绝，Agent 会继续使用原本的生图工具。
...
`init` 默认引导配置 GPT Image 2 + OpenAI-compatible 图像网关。如果当前 Codex 或宿主 Agent 已经暴露内置图像生成工具，BananaHub 也可以把它作为 host-native 通道使用，不需要在本地保存额外 API key。
...
| `host-native` | 本地 provider 不完整，但 Codex 或宿主 Agent 有图像工具 | 优化 prompt，必要时归档，然后交给宿主图像工具出图 |
| `prompt-only` | 没有可用 provider，也没有宿主图像工具 | 只产出高质量 prompt；不会假装已经出图 |

Codex OAuth 和部分 API 登录模式会暴露内置图像生成工具。CLI 里可以用 `BANANAHUB_HOST_IMAGEGEN=1` 或 `check-mode --host-imagegen` 显式声明宿主有图像工具。
...
| `/bananahub test-host-imagegen` | 验证 Codex / 宿主内置图像生成工具 |
...
| `/bananahub capture-workflow` | 把当前多轮调图过程沉淀成可复用 workflow 草案 |
| `/bananahub init` | 检查和初始化环境 |

不带 `/bananahub` 的普通生图请求默认只把 BananaHub 当作优化建议层：先问再优化，不会静默替换宿主生图工具。
...
多轮调图结束后，可以用 `/bananahub capture-workflow` 把最终满意版本、有效 prompt、失败尝试、锁定项、每轮允许变化、模型/通道选择整理成 `type: workflow` 模板草案。它会先询问再写文件，可保存到用户模板、当前项目或可发布的模板目录。
