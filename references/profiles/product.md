...
- Default: "three-point softbox lighting setup to highlight texture and form"
...
### Camera Angle
- Default: "slightly elevated 45-degree angle" (most versatile product angle)
- Available angles:
...
### Surface & Background
- No background specified → infer from product type:
...
**Enhanced result**: Create a photorealistic product shot of Bluetooth headphones on a clean white background, three-point softbox lighting setup to highlight form and texture, slightly elevated 45-degree angle, matte plastic housing with brushed aluminum accents, sharp focus on product details, isolated product with no distractions, professional catalog photography style
...
- For e-commerce, suggest 1:1 ratio and white background unless told otherwise — these are platform requirements
- When user mentions a specific platform (淘宝、小红书、亚马逊), adjust styling to match that platform's conventions
- Material descriptions are critical for product shots — always check if key materials can be specified
